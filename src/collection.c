...
static inline int collection_darray_resize(DArray *array, size_t new_size)
{
    array->max = new_size;
...
int collection_darray_expand(DArray *array)
{
    ...
    check(collection_darray_resize(array, array->max + array->expand_rate) == 0,
...
int collection_darray_contract(DArray *array)
{
    ...
    return collection_darray_resize(array, new_size + 1);
}
...
int collection_darray_push(DArray *array, void *element)
{
    ...
    if (DYNAMIC_ARRAY_END(array) >= DYNAMIC_ARRAY_MAX(array)) {
        return collection_darray_expand(array);
    } else {
        return 0;
    }
}
...
void *collection_darray_pop(DArray *array)
{
    ...
    void *element = collection_darray_remove(array, array->end - 1);
    array->end--;
...
Hashmap *collection_hashmap_create(HashmapCompare compare, HashmapHash hash)
{
    ...
    map->buckets = collection_darray_create(sizeof(DArray *), DEFAULT_BUCKETS_NUM);
...
error:
    if (map) collection_hashmap_destroy(map);
    return NULL;
}
    ...
    if (map) {
        if (map->buckets) {
            for (i = 0; i < DYNAMIC_ARRAY_COUNT(map->buckets); i++) {
                DArray *bucket = collection_darray_get(map->buckets, i);
                if (bucket) {
                    for (j = 0; j < DYNAMIC_ARRAY_COUNT(bucket); j++) {
                        free(collection_darray_get(bucket, j));
                    }
                    collection_darray_destroy(bucket);
                }
            }
            collection_darray_destroy(map->buckets);
        }
        free(map);
    }
}
...
static inline DArray *find_bucket(Hashmap *map, void *key,
        int create,
        uint32_t *hash_out)
{
    ...
    *hash_out = hash;
    ...
    DArray *bucket = collection_darray_get(map->buckets, bucket_n);
    if (!bucket && create) {
        // new bucket, set it up
        bucket = collection_darray_create(sizeof(void *), DEFAULT_BUCKETS_NUM);
        check_mem(bucket);
        collection_darray_set(map->buckets, bucket_n, bucket);
    }
    return bucket;
error:
    return NULL;
...
int collection_hashmap_set(Hashmap *map, void *key, void *value)
{
    ...
    collection_darray_push(bucket, node);
    return 0;
error:
    return -1;
}

static inline int get_node(Hashmap *map, uint32_t hash, DArray *bucket,
        void *key)
{
    int i = 0;
    ...
    for (i = 0; i < DYNAMIC_ARRAY_END(bucket); i++) {
        debug("TRY: %d", i);
        HashmapNode *node = collection_darray_get(bucket, i);
...
int collection_hashmap_traverse(Hashmap *map, HashmapTraverseCb traverse_cb)
{
    ...
    for (i = 0; i < DYNAMIC_ARRAY_COUNT(map->buckets); i++) {
        ...
        if (bucket) {
            for (j = 0; j < DYNAMIC_ARRAY_COUNT(bucket); j++) {
                HashmapNode *node = collection_darray_get(bucket, j);
                rc = traverse_cb(node);
                if (rc != 0) return rc;
            }
        }
...
void *collection_hashmap_delete(Hashmap *map, void *key)
{
    ...
    HashmapNode *ending = collection_darray_pop(bucket);

    if (ending != node) {
        // looks like it's not the last one, swap it
        collection_darray_set(bucket, i, ending);
    }
    return value;
}
