...
int add_watch_directory(Hashmap *map, int fd, char *dirname)
{
    ...
    int ret = collection_hashmap_set(map, &wd, dirname);
    ...
    for (i = 0; i < DYNAMIC_ARRAY_COUNT(map->buckets); i++) {
        ...
        if (bucket) {
            for (j = 0; j < DYNAMIC_ARRAY_COUNT(bucket); j++) {
                ...
                inotify_rm_watch(fd, (intptr_t)node->key);
                collection_hashmap_delete(map, node->key);
            }
        }
    }
    close(fd);
...
void notif_watching_loop_start(int dir_count, char **paths)
{
    int fd, i, len;

    keep_running = 1;
    // Map keeps track of watch dirs as keys and its names as values
    Hashmap *map = collection_hashmap_create(NULL, NULL);
    ...
    int client_fd = connection_client_get_socket();
    ...
    while (keep_running) {
        ...
        while (j < len) {
            ...
            if (event->len) {
                char *parentdir = collection_hashmap_get(map, &event->wd);
                if (event->mask & IN_CREATE) {
                    if (event->mask & IN_ISDIR) {
                        ...
                        connection_send_msg(client_fd, message);
